...
# Pre-select: already registered user-scope servers first, then new recommended ones
SELECTED_MCPS=()
...
# Pre-select: already-enabled plugins from settings.json, falling back to recommended
SELECTED_PLUGINS=()
_already_enabled=$(python3 -c "
import json, os
...
if [[ -n "$_already_enabled" ]]; then
  for _i in "${!PLUGIN_IDS[@]}"; do
    echo "$_already_enabled" | grep -qx "${PLUGIN_IDS[$_i]}" && SELECTED_PLUGINS+=("${PLUGIN_IDS[$_i]}")
  done
else
  # First run — default to recommended
  for _i in "${!PLUGIN_IDS[@]}"; do
