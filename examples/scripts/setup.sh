...
# Writes .mcp.json, .env, and settings.json to ~/.claude/ (global — no project path needed).
...
if [[ -d "$OCC_DIR/.git" ]]; then
  ...
  if git -C "$OCC_DIR" pull --quiet 2>/dev/null; then
    ...
    echo "  ⚠️   Pull failed — OCC may be in a broken state."
    echo "  Auto-healing: re-cloning OCC..."
    rm -rf "$OCC_DIR"
...
cp -r "$SCRIPT_DIR/.claude/personas/"*.md "$CLAUDE_DIR/personas/" 2>/dev/null || true
cp -r "$SCRIPT_DIR/.claude/commands/"*.md "$CLAUDE_DIR/commands/" 2>/dev/null || true
echo "  Done (global ~/.claude/ — available in all sessions)."

# ── Step 2: Select MCP servers ──────────────────────────────────
echo ""
echo "[2/8] Select MCP servers to install"
echo "  Recommended servers are pre-selected (* = recommended)."
echo "  Toggle by number, 'r' recommended, 'a' all, 'n' none, or Enter to continue."

# Build MCP arrays from catalog
MCP_IDS=(); MCP_NAMES=(); MCP_RECS=()
while IFS='|' read -r _rec _mid _name _desc _keys; do
  MCP_IDS+=("$_mid")
...
for mid, m in sorted(c['mcpServers'].items(), key=lambda x: (not x[1]['recommended'], x[1]['name'])):
    rec = '*' if m['recommended'] else ' '
    keys = ', '.join(m.get('requiredKeys', []))
    print(f'{rec}|{mid}|{m[\"name\"]}|{m[\"description\"]}|{keys}')
")

# Pre-select: existing .mcp.json entries first, then any new recommended servers not already selected
SELECTED_MCPS=()
if [[ -f "$MCP_FILE" ]]; then
  while IFS= read -r _mid; do
    for _id in "${MCP_IDS[@]}"; do [[ "$_id" == "$_mid" ]] && SELECTED_MCPS+=("$_mid") && break; done
  done < <(python3 -c "import json,sys; d=json.load(open('$MCP_FILE')); [print(k) for k in d.get('mcpServers',{})]")
fi
...
while true; do
  ...
  for _i in "${!MCP_IDS[@]}"; do
    ...
    _star=""; [[ "${MCP_RECS[$_i]}" == "*" ]] && _star=" *"
    printf "  %s %2d) %s%s\n" "$_mark" "$((_i+1))" "${MCP_NAMES[$_i]}" "$_star"
  done
  echo ""
...
echo "  Recommended plugins are pre-selected. Toggle or Enter to continue."

# Build plugin arrays from catalog
PLUGIN_IDS=(); PLUGIN_NAMES=(); PLUGIN_RECS=()
while IFS='|' read -r _rec _pid _name _desc _cat; do
  PLUGIN_IDS+=("$_pid")
...
for _i in "${!PLUGIN_IDS[@]}"; do
  [[ "${PLUGIN_RECS[$_i]}" == "*" ]] && SELECTED_PLUGINS+=("${PLUGIN_IDS[$_i]}")
done
...
while true; do
  ...
  for _i in "${!PLUGIN_IDS[@]}"; do
    ...
    _mark="[ ]"; [[ $_in_sel -eq 1 ]] && _mark="[x]"
    _star=""; [[ "${PLUGIN_RECS[$_i]}" == "*" ]] && _star=" *"
    printf "  %s %2d) %s%s\n" "$_mark" "$((_i+1))" "${PLUGIN_NAMES[$_i]}" "$_star"
  done
  echo ""
  printf "  Toggle (numbers), r=recommended, a=all, n=none, Enter=done: "
  read -r _sel
  ...
  case "$_sel" in
    r|R)
      SELECTED_PLUGINS=()
      for _i in "${!PLUGIN_IDS[@]}"; do [[ "${PLUGIN_RECS[$_i]}" == "*" ]] && SELECTED_PLUGINS+=("${PLUGIN_IDS[$_i]}"); done
      ;;
    a|A) SELECTED_PLUGINS=("${PLUGIN_IDS[@]}") ;;
    n|N) SELECTED_PLUGINS=() ;;
    *)
      for _num in $_sel; do
        if [[ "$_num" =~ ^[0-9]+$ ]] && (( _num >= 1 && _num <= ${#PLUGIN_IDS[@]} )); then
          _id="${PLUGIN_IDS[$(( _num-1 ))]}"
          _found=0; _new=()
...
_step4_py=$(mktemp /tmp/occ_step4_XXXXX.py)
cat > "$_step4_py" << 'PYEOF'
...
if [[ $_step4_rc -ne 0 ]]; then
  echo "  ❌ Step 4 failed (exit $_step4_rc) — check output above."
  exit 1
fi
...
if [[ "$_clickup_oauth" == "true" ]] \
  ...
  _cu_py=$(mktemp /tmp/cu_oauth_XXXXX.py)
  cat > "$_cu_py" << 'CUEOF'
...
# ── Step 5: Generate .mcp.json from selected servers ────────────
echo ""
echo "[5/8] Generating .mcp.json..."

python3 - "$CATALOG" "$MCP_FILE" "$ENV_FILE" "${SELECTED_MCPS[@]+"${SELECTED_MCPS[@]}"}" <<'PYEOF'
import json, sys, os
...
# Build selected servers from catalog
enabled = []
...
if [[ -f "$MULTI_CHECK_CONFIG" ]]; then
  ...
  if [[ "$(echo "$MULTI_ANSWER" | tr '[:upper:]' '[:lower:]')" == "n" ]]; then
    ...
    echo ""
    bash "$SCRIPT_DIR/scripts/setup-multi-system.sh"
  fi
fi
...
echo "  ~/.claude/.mcp.json          — MCP server config (chmod 600)"
