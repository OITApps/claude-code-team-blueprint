...
PLUGIN_IDS=(); PLUGIN_NAMES=(); PLUGIN_RECS=(); PLUGIN_LOCKED=()
...
if [[ -n "$_already_enabled" ]]; then
  for _i in "${!PLUGIN_IDS[@]}"; do
    if echo "$_already_enabled" | grep -qx "${PLUGIN_IDS[$_i]}" || [[ "${PLUGIN_LOCKED[$_i]}" == "1" ]]; then
...
while true; do
  ...
  for _i in "${!PLUGIN_IDS[@]}"; do
    ...
    _lock=""; [[ "${PLUGIN_LOCKED[$_i]}" == "1" ]] && _lock=" (always on)"
    printf "  %s %2d) %s%s%s\n" "$_mark" "$((_i+1))" "${PLUGIN_NAMES[$_i]}" "$_star" "$_lock"
  done
  echo ""
  printf "  Toggle (numbers), r=recommended, a=all, n=none, Enter=done: "
  read -r _sel
