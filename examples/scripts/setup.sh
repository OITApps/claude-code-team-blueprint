...
echo "[1/9] Copying personas and commands..."
_step_start "Personas and commands copied"
if ! mkdir -p "$CLAUDE_DIR/commands" "$CLAUDE_DIR/personas" 2>/dev/null; then
  step_fail "Personas/commands" "could not create directories under $CLAUDE_DIR"
...
if not selected:
    ...
    for mid, m in sorted(catalog['mcpServers'].items()):
        ...
        # e.g. ClickUp: CLICKUP_API_KEY already set means OAuth was already completed
...
# ── Shared secret distribution (optional) ────────────────────────
echo ""
_read_input -rp "  Would you like to fetch shared team credentials? (requires GitHub auth) [y/N] " _fetch_secrets
if [[ "$(echo "$_fetch_secrets" | tr '[:upper:]' '[:lower:]')" == "y" ]]; then
  FETCH_SCRIPT="$SCRIPT_DIR/scripts/fetch-shared-secrets.sh"
  if [[ -f "$FETCH_SCRIPT" ]]; then
    ...
    if [[ $_fetch_rc -eq 0 ]]; then
      step_ok "Shared secrets fetched"
      set -a; source "$ENV_FILE" 2>/dev/null; set +a
    else
      step_warn "Shared secrets" "fetch did not complete (exit code $_fetch_rc)"
    fi
  else
    step_warn "Shared secrets" "fetch script not found at $FETCH_SCRIPT"
  fi
else
  echo "  Skipped shared secret fetch."
fi
...
echo "  Sync Claude memories (feedback, preferences, references) across machines"
echo "  via the OCC repo. Sensitive content (keys, tokens, secrets) is never synced."
echo ""

SYNC_SCRIPT="$SCRIPT_DIR/scripts/sync-memory.sh"

if [[ -f "$SYNC_SCRIPT" ]]; then
  read -rp "  Would you like to sync preferences from a previous machine? [y/N] " SYNC_ANSWER
  if [[ "$(echo "$SYNC_ANSWER" | tr '[:upper:]' '[:lower:]')" == "y" ]]; then
    echo ""
    if bash "$SYNC_SCRIPT" pull; then
      step_ok "Preferences synced from repo"
    else
      step_warn "Preference sync" "pull did not complete"
    fi
  else
    echo "  Skipped. To sync later: $SYNC_SCRIPT push|pull"
    step_ok "Preference sync (skipped)"
  fi
else
