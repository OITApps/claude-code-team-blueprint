...
add_server() {
  ...
  if [[ ! -t 0 ]]; then
    ...
    # Detect IDE: env vars (when available) OR code/cursor CLI on PATH
    local _in_ide=false
    if [[ -n "${VSCODE_IPC_HOOK_CLI:-}" ]] || [[ -n "${CURSOR_TRACE_DIR:-}" ]] || [[ "${TERM_PROGRAM:-}" =~ ^(vscode|cursor)$ ]]; then
      _in_ide=true
    elif command -v code &>/dev/null && [[ "$(uname)" == "Darwin" ]]; then
      # Claude Code Bash tool doesn't inherit VSCode env vars,
      # but if `code` CLI exists, user likely has VSCode — prefer its terminal
      _in_ide=true
    fi
